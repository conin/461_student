...
TEST( Allocate_3_middle_up_free , MemoryShakeOut )
{
	io::create("Student Values", "Test8");

	fprintf(io::getHandle(),"Test8: \n\n");

	fprintf(io::getHandle()," 1) used                  \n");
	fprintf(io::getHandle()," 2) used                  \n");
	fprintf(io::getHandle()," 3) used                  \n");
   	fprintf(io::getHandle()," 4) free middle up        \n\n");
	...
	// ---- Verify B: -----------------------------------------------------------
		...
		CHECK_EQUAL( PTR_FIX(h->usedHead), 0x40 + 2*(0x10 + 0x200));
	...
	// ---- HDR 2 -------------------------------------------
		...
		CHECK_EQUAL( free->freePrev, 0x0 );
		// Next
		CHECK_EQUAL( PTR_FIX(free->freeNext), 0x40 + 3*(0x10+0x200) );
		// Hdr Size
		CHECK_EQUAL( hdrEnd - hdrStart, sizeof(usedHdr) );
	...
	// ---- Used Block ----------------------------------------------------
		...
		CHECK_EQUAL( PTR_FIX(blkStart), PTR_FIX(hdrEnd) );
		CHECK_EQUAL( PTR_FIX(blkEnd), 0x670);
		CHECK_EQUAL( used->blockSize, 0x200 );
	...
	// ---- HDR 4 -------------------------------------------
		...
		CHECK_EQUAL( PTR_FIX(hdrEnd), 0x680 );
		// Prev
		CHECK_EQUAL( PTR_FIX(free->freePrev), 0x250 );
		// Next
		CHECK_EQUAL( free->freeNext, 0x0 );
		// Hdr Size
		CHECK_EQUAL( hdrEnd - hdrStart, sizeof(freeHdr) );
	...
	// ---- Verify C: -----------------------------------------------------------
		...
		CHECK_EQUAL( PTR_FIX(h->usedHead), 0x40 + 0x420 );
	...
	// ---- HDR 1 -------------------------------------------
		...
		CHECK_EQUAL( free->freePrev, 0x0 );
		// Next
		CHECK_EQUAL( PTR_FIX(free->freeNext), 0x40+0x420+0x210 );
		// Hdr Size
		CHECK_EQUAL( hdrEnd - hdrStart, sizeof(freeHdr) );
