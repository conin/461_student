...
TEST( Allocate_3_100_middle_up_free , MemoryShakeOut )
{
	io::create("Student Values", "Test10");


	fprintf(io::getHandle(),"Test10: \n\n");
	...
	fprintf(io::getHandle()," 3) used     (make sure it's consumes 100 percent of heap)             \n");
   	...
   	fprintf(io::getHandle()," 4) middle up   \n\n");
	...
	fprintf(io::getHandle(),"     mem.InitializeSystem();            \n");
   	...
   	fprintf(io::getHandle(),"     void *b = mem.Malloc( 0x4290 );       \n");
	fprintf(io::getHandle(),"     void *c = mem.Malloc( 0x4280 );       \n");
	...
	// ---- Verify A: -----------------------------------------------------------
		...
		// Used / Free
		CHECK_EQUAL( h->freeHead, 0x0);
		CHECK_EQUAL( PTR_FIX(h->usedHead), 0x8570 );
	...
	// ---- USED HDR 1 -------------------------------------------
		...
		// Above Free
		CHECK_EQUAL(used->aboveBlockFree, false);
	...
	// ---- Used HDR 2 -------------------------------------------
		...
		// Prev
		CHECK_EQUAL( PTR_FIX(used->usedPrev), 0x8570 );
		// Next
	...
	// ---- used BLOCK ----------------------------------------------------
		...
		CHECK_EQUAL( PTR_FIX(blkStart), PTR_FIX(hdrEnd) );
		CHECK_EQUAL( PTR_FIX(blkEnd), 0x8570);
	...
	// ---- Used HDR 3 -------------------------------------------
		...
		// Hdr Start
		CHECK_EQUAL( PTR_FIX(hdrStart), 0x8570 );
		// Hdr End
		CHECK_EQUAL( PTR_FIX(hdrEnd), 0x8580 );
		// Prev
		CHECK_EQUAL( used->usedPrev, 0);
		// Next
	...
	// ---- Heap Stats ------------------------------------------------------
		...
		CHECK_EQUAL( h->stats.currUsedMem, 0xc790 - 0x4290 );
		
		CHECK_EQUAL( h->stats.currNumFreeBlocks, 1 );
		CHECK_EQUAL( h->stats.currFreeMem, 0x4290 );
	...
	// ---- HDR 2 -------------------------------------------
		...
		// Above Free
		CHECK_EQUAL(free->aboveBlockFree, false);
		...
		CHECK_EQUAL( PTR_FIX(hdrEnd), 0x42e0 );
		// Prev
		CHECK_EQUAL( free->freePrev, 0x0 );
		// Next
		CHECK_EQUAL( free->freeNext, 0x0 );
		// Hdr Size
	...
	// ---- Free BLOCK ----------------------------------------------------
		...
		CHECK_EQUAL( PTR_FIX(blkStart), PTR_FIX(hdrEnd) );
		CHECK_EQUAL( PTR_FIX(blkEnd), 0x8570 );
		CHECK_EQUAL( free->blockSize, 0x4290 );
