...
TEST( Allocate_3_100_bottom_up_free , MemoryShakeOut )
{
	io::create("Student Values", "Test12");

	fprintf(io::getHandle(),"Test12: \n\n");
	...
	fprintf(io::getHandle()," 3) used     (make sure it's consumes 100 percent of heap)             \n");
   	...
   	fprintf(io::getHandle()," 4) bottom up   \n\n");
	...
	// ---- Verify B: -----------------------------------------------------------
		...
		// Used / Free
		CHECK_EQUAL( PTR_FIX(h->freeHead), 0x8560 );
		CHECK_EQUAL( PTR_FIX(h->usedHead), 0x42d0 );
	...
	// ---- HDR 2 -------------------------------------------
		...
		CHECK_EQUAL( PTR_FIX(hdrEnd), 0x42e0 );
		// Prev
		CHECK_EQUAL( free->freePrev, 0);
		// Next
		CHECK_EQUAL( free->freeNext, 0 );
		// Hdr Size
		CHECK_EQUAL( hdrEnd - hdrStart, sizeof(freeHdr) );
