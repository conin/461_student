...
memVoid memSystem::mergeBlocks(freeHdr * ptmp, memU32)
{
	...
	if (down != 0 && down->blockType == 0xCC && down <= this->pHeap->stats.heapBottomAddr)
	{
		...
		if ((unsigned int*)(((memU32)ptmp + sizeof(freeHdr) + ptmp->blockSize) - sizeof(memU32)) < this->pHeap->stats.heapBottomAddr)
		{ 
	...
	if (ptmp->aboveBlockFree && s >= this->pHeap->stats.heapTopAddr)
	{
		...
		if((unsigned int*)(((memU32)up + sizeof(freeHdr) + up->blockSize) - sizeof(memU32)) < this->pHeap->stats.heapBottomAddr)
		{
...
memVoid * memSystem::Malloc(const memU32 size)
{
	...
	else
	{
		...
		if (fr > pHeap->stats.heapBottomAddr && fr < pHeap->stats.heapTopAddr)
			Trace::out("caution please look at this");
