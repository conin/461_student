...
public:
	...
	virtual void runIndivTest(UnitData &, UnitStats &) = 0;
...
public:
	...
	static void RunTests()
	{
		...
		while (pTmp != 0)
		{
			...
			// run the test
			pRegistry->tr_data.result = true;
			
			pTest->testFunc->runIndivTest(pRegistry->tr_data, pRegistry->tr_stats);
		
			

			if (pRegistry->tr_data.result == true)
			{
				pRegistry->tr_stats.testPass++;
			}
			else
			{
				pRegistry->tr_stats.testFail++;
			}
		...
		UnitTrace::out("indvChecks: %d\n", pRegistry->tr_stats.indvAsserts);
	}
...
{ \
	tr_stats.indvAsserts++;\
	if( !( condition ) ) \
	{ \
		tr_data.result = false;  \
		tr_data.pMemberName = this->pName; \
		tr_data.pSourceFilePath = __FILE__; \
		tr_data.sourceLineNumber = __LINE__; \
		UnitTrace::out("%s(%d): %s\n", tr_data.pSourceFilePath, tr_data.sourceLineNumber, tr_data.pMemberName  ); \
		return; \
	} \
	else\
	{\
...
{ \
	...
	\
	void runIndivTest(UnitData &, UnitStats &);\
} TestName##GroupName##_instance; \
\
void TestName##GroupName##_Test::runIndivTest( UnitData &tr_data, UnitStats &tr_stats ) 
